...
const ConversationPanel = ({ agent }: ConversationPanelProps) => {
  ...
  const loadMessages = async (reset: boolean = false) => {
    ...
    try {
      const startIndex = reset ? 0 : messages.length;
      
      const fetchedMessages = await S3MessageFetcher.fetchAgentMessages(agent.id, startIndex);
      ...
      setHasMoreMessages(fetchedMessages.length >= 10); // Assume more if we got a full batch
      
      setMessageCount(prev => reset ? fetchedMessages.length : prev + fetchedMessages.length);
